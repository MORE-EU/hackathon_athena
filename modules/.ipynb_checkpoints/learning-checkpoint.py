...
def predict_one_datapoint(df_test, model, feats, target):
    """
    Applies a regression model to predict values of a dependent variable for a given dataframe and 
    given features.
...
def perform_grid_search(df, feats, target, scorer, model, params, randomized=False, cv=3, n_jobs=-1, n_iter=100):
    ...
    if randomized == True:
        grid_pipeline = RandomizedSearchCV(model, params, n_iter=n_iter, verbose=0, n_jobs=n_jobs, pre_dispatch=64, scoring=scorer, cv=cv, random_state=42).fit(X, y)
    else:
        grid_pipeline = GridSearchCV(model, params, verbose=0, n_jobs=n_jobs, pre_dispatch=32, scoring=scorer, cv=cv).fit(X, y)
